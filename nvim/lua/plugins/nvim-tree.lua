return {
    ...
    config = function()
        ...
        local function live_grep_bellow_node()
            ...
            ts.live_grep({
                ...
                attach_mappings = function(prompt_bufnr, map)
                    map(
                        { "i", "n" },
                        "<cr>",
                        open_telescope_entry_with_nvim_tree(prompt_bufnr, live_grep_entry_to_path)
                    )
                    return true
                end,
            })
        ...
        local function find_file_bellow_node()
            local node = api.tree.get_node_under_cursor()
            ts.fd({
                cwd = to_dir(node.absolute_path),
                attach_mappings = function(prompt_bufnr, map)
                    map(
                        { "i", "n" },
                        "<cr>",
                        open_telescope_entry_with_nvim_tree(prompt_bufnr, find_file_entry_to_path)
                    )
                    return true
                end,
            })
        ...
        tree.setup({
            ...
            on_attach = function(bufnr)
                ...
                n({
                    ...
                    ["<c-f>"] = with_help(live_grep_bellow_node, "live grep bellow node"),
                    ["<c-p>"] = with_help(find_file_bellow_node, "find file bellow node"),
                    ["<c-t>"] = with_help(open_term_in_file_dir, "open term this dir"),
                })
            end,
            view = {
                relativenumber = true,
